/* NIS calc-engine — interactive scoring for generated tool pages. */
(function () {
  "use strict";
  ...
  function scoreOf(inputId) {
    ...
    if (inp.type === "select" && inp.options) {
      var opt = inp.options.filter(function (item) { return Number(item.value) === Number(val); })[0];
  ...
  function computeTotal() {
    ...
    if (formulaCfg.type === "expression" && formulaCfg.expression) {
      ...
      inputsCfg.forEach(function (inp) {
        expr = expr.replace(new RegExp("\\b" + inp.id + "\\b", "g"), String(Number(state[inp.id]) || 0));
      });
  ...
  function renderAll() {
    ...
    if (valueEl) valueEl.textContent = totalStr;
  ...
  function readField(el, inp) {
    var raw = el.value;
    state[inp.id] = inp.type === "number" ? parseFloat(raw) || 0 : parseFloat(raw);
  }
