...
describe("CI inspect preserves Drive HOLD index and policy packs", () => {
  ...
  it("generate_layers keeps Drive HOLD extra keys including Camada 43", () => {
    ...
    assert.match(src, /chatgpt_wave_docs_census/);
    assert.match(src, /chatgpt_wave0_nested_census/);
...
describe("ChatGPT Arquivo Mestre nested census HOLD", () => {
  it("names nested KEEP children without closing B9, freeze, MD/REG or ingesting zips", () => {
    ...
    assert.equal(catalog.itemCount, catalog.items.length);
    assert.equal(catalog.deployedCount, catalog.items.filter((i) => i.deployed === true).length);
    assert.equal(catalog.itemCount, 191);
    assert.equal(catalog.deployedCount, 86);
...
describe("ChatGPT wave docs census HOLD", () => {
  it("names 2026-08-24 wave-folder children without closing B9 or ingesting binaries", () => {
    const stamp = JSON.parse(readFileSync(join(gatePub, "drive/CKO-CHATGPT-WAVE-DOCS-CENSUS-HOLD.json"), "utf8"));
    const catalog = JSON.parse(readFileSync(join(gatePub, "drive/catalog.json"), "utf8"));
    const gateLayers = JSON.parse(readFileSync(join(gatePub, "data/layers.json"), "utf8"));
    assert.equal(stamp.release_allowed, false);
    assert.equal(stamp.closes_b9, false);
    ...
    assert.equal(stamp.freeze_gate, "DENY");
    assert.equal(stamp.zip_ingested, false);
    assert.equal(stamp.xlsx_ingested, false);
    assert.equal(stamp.docx_ingested, false);
    assert.equal(stamp.pdf_ingested, false);
    assert.equal(stamp.wave_closures_folder_is_not_a_close, true);
    assert.equal(stamp.mod19_xlsx_is_not_layer43_close, true);
    assert.equal(stamp.named_children_n, 49);
    assert.equal(stamp.hosted_small_n, 0);
    assert.equal(stamp.modules_n, 20);
    assert.equal(stamp.canonical_navy, "#1A3E74");
    assert.equal(stamp.canonical["LYR-PUB-001"], "1c3o8yuNMnjyzZI84kLdDqLwCGSJtRb-W");
    const modules = stamp.wave_folders.find((f) => f.id === "1-A_Seji4h49JxEFh8XYYiZu1yCPYCDVQ");
    assert.equal(modules.children_n, 20);
    assert.equal(modules.children.length, 20);
    const waveClosures = stamp.wave_folders.find((f) => f.id === "14bIDUd_kZYrYIg_0SyelUpEHIS7S9kWU");
    assert.equal(waveClosures.name, "06_WAVE_CLOSURES");
    assert.equal(waveClosures.children_n, 1);
    assert.equal(waveClosures.not_a_close, true);
    assert.equal(stamp.mod19.id, "1_1qloL9yYBv1ykvOxhKyCYqkuy-3ECP8");
    assert.equal(stamp.mod19.is_layer43_close, false);
    assert.equal(gateLayers.chatgpt_wave_docs_census.named_children_n, 49);
    assert.equal(gateLayers.chatgpt_wave_docs_census.hosted_small_n, 0);
    assert.equal(gateLayers.chatgpt_wave_docs_census.freeze_gate, "DENY");
    assert.equal(gateLayers.chatgpt_wave_docs_census.closes_b9, false);
    assert.equal(gateLayers.chatgpt_wave_docs_census.md_reg_complete, false);
    const pub = gateLayers.layers.find((l) => l.id === "LYR-PUB-001");
    assert.equal(pub.drive_id, "1c3o8yuNMnjyzZI84kLdDqLwCGSJtRb-W");
    assert.equal(catalog.itemCount, catalog.items.length);
    assert.equal(catalog.deployedCount, catalog.items.filter((i) => i.deployed === true).length);
    assert.equal(catalog.itemCount, 191);
    assert.equal(catalog.deployedCount, 86);
    assert.equal(catalog.items.find((i) => i.id === "14bIDUd_kZYrYIg_0SyelUpEHIS7S9kWU").deployed, true);
    assert.equal(catalog.items.find((i) => i.id === "1-A_Seji4h49JxEFh8XYYiZu1yCPYCDVQ").deployed, true);
    const html = readFileSync(join(site, "cko-estado.html"), "utf8");
    assert.match(html, /Censo documentação ChatGPT 2026-08-24/);
    assert.match(html, /06_WAVE_CLOSURES/);
    assert.match(html, /MOD-19/);
    assert.match(html, /hosted_small_n: 0/);
    assert.match(html, /closes_b9: false/);
    assert.equal(html.includes("md_reg_complete: true"), false);
    assert.equal(html.includes("implantado: true"), false);
  });
});

describe("ChatGPT Wave0 nested census HOLD", () => {
  it("names SUPERSEDED and WAVE_CLOSURES Wave0 children without closing B9", () => {
    const stamp = JSON.parse(readFileSync(join(gatePub, "drive/CKO-CHATGPT-WAVE0-NESTED-CENSUS-HOLD.json"), "utf8"));
    const catalog = JSON.parse(readFileSync(join(gatePub, "drive/catalog.json"), "utf8"));
    const gateLayers = JSON.parse(readFileSync(join(gatePub, "data/layers.json"), "utf8"));
    assert.equal(stamp.release_allowed, false);
    assert.equal(stamp.closes_b9, false);
    ...
    assert.equal(stamp.docx_ingested, false);
    assert.equal(stamp.wave_closures_folder_is_not_a_close, true);
    assert.equal(stamp.superseded_folder_is_not_a_close, true);
    assert.equal(stamp.filename_closure_is_not_a_close, true);
    assert.equal(stamp.named_children_n, 15);
    assert.equal(stamp.hosted_small_n, 0);
    assert.equal(stamp.canonical["LYR-PUB-001"], "1c3o8yuNMnjyzZI84kLdDqLwCGSJtRb-W");
    const superseded = stamp.folders.find((f) => f.id === "1kY6aMufy5M7l9c8HJ1QiSJ4fiwjz3c3S");
    assert.equal(superseded.children_n, 4);
    assert.equal(superseded.superseded, true);
    assert.equal(superseded.not_a_close, true);
    const nested = stamp.folders.find((f) => f.id === "1PoYOBY7HiIJBLutnvgpzX1rd_t4bHj79");
    assert.equal(nested.children_n, 11);
    assert.equal(nested.has_v1_0_and_v1_0_1, true);
    assert.equal(nested.not_a_close, true);
    assert.equal(gateLayers.chatgpt_wave0_nested_census.named_children_n, 15);
    assert.equal(gateLayers.chatgpt_wave0_nested_census.hosted_small_n, 0);
    assert.equal(gateLayers.chatgpt_wave0_nested_census.freeze_gate, "DENY");
    assert.equal(gateLayers.chatgpt_wave0_nested_census.closes_b9, false);
    const pub = gateLayers.layers.find((l) => l.id === "LYR-PUB-001");
    assert.equal(pub.drive_id, "1c3o8yuNMnjyzZI84kLdDqLwCGSJtRb-W");
    assert.equal(catalog.itemCount, catalog.items.length);
    assert.equal(catalog.deployedCount, catalog.items.filter((i) => i.deployed === true).length);
    assert.equal(catalog.itemCount, 191);
    assert.equal(catalog.deployedCount, 86);
    assert.equal(catalog.items.find((i) => i.id === "1kY6aMufy5M7l9c8HJ1QiSJ4fiwjz3c3S").deployed, true);
    assert.equal(catalog.items.find((i) => i.id === "1PoYOBY7HiIJBLutnvgpzX1rd_t4bHj79").deployed, true);
    const html = readFileSync(join(site, "cko-estado.html"), "utf8");
    assert.match(html, /Censo aninhado Wave0 ChatGPT/);
    assert.match(html, /SUPERSEDED/);
    assert.match(html, /06_WAVE_CLOSURES/);
    assert.match(html, /named_children_n: 15/);
    assert.match(html, /hosted_small_n: 0/);
    assert.match(html, /closes_b9: false/);
    assert.equal(html.includes("md_reg_complete: true"), false);
    assert.equal(html.includes("implantado: true"), false);
...
describe("Rights decision form HOLD", () => {
  it("records bucket APPROVED without naming slots, zeroing rights_holds, or closing B9", () => {
    ...
    const residual = JSON.parse(readFileSync(join(gatePub, "data/residual-uncertainty.json"), "utf8"));
    const html = readFileSync(join(site, "cko-estado.html"), "utf8");
    assert.equal(stamp.release_allowed, false);
    assert.equal(stamp.closes_b9, false);
    assert.equal(stamp.closes_rights_count_13, false);
    assert.equal(stamp.provenance_per_media, false);
    ...
    assert.equal(stamp.bucket.verdict, "APPROVED");
    assert.equal(stamp.bucket.mapped_choice, "CLEAR");
    ...
    assert.equal(stamp.recorded_human["HOLD-HUMAN-RIGHTS-CHAIN"].verdict, "APPROVED");
    assert.equal(stamp.recorded_human["HOLD-HUMAN-RIGHTS-CHAIN"].closes_rights_count_13, false);
    assert.equal(stamp.clinical_promotion, "DENIED");
    assert.equal(stamp.calculators_scales, "PAUSED");
    assert.ok(stamp.named_rights_adjacent_not_the_13_titles.every((s) => s.verdict === "APPROVED"));
    ...
    assert.equal(rights.recorded_human.verdict, "APPROVED");
    assert.equal(rights.recorded_human.closes_rights_count_13, false);
    assert.equal(rights.recorded_human.closes_b9, false);
    assert.equal(rights.modality, "MUST_NOT");
    const hero = human.items.find((i) => i.id === "HOLD-HUMAN-HERO-MEDIA-RIGHTS");
    assert.equal(hero.recorded_human.verdict, "APPROVED");
    assert.equal(hero.recorded_human.publishes_hero_layer_7, false);
    assert.equal(residual.open_counts.rights_holds, 13);
    assert.match(html, /13 RIGHTS_PROVENANCE/);
    assert.match(html, /bucket aprovado/);
    assert.match(html, /closes_rights_count_13: false/);
