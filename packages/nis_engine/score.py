"""Pure scoring for tool JSON (sum and arithmetic expression)."""
...
def score_of(inp: Mapping[str, Any], value: Any) -> float:
    ...
    if inp.get("type") == "select":
        for opt in inp.get("options") or []:
            if _as_number(opt.get("value")) == _as_number(value):
...
def compute(tool: Mapping[str, Any], values: Mapping[str, Any] | None = None) -> float:
    ...
    if ftype == "expression":
        ...
        for inp in inputs:
            expr = re.sub(rf"\b{re.escape(inp['id'])}\b", str(_as_number(state.get(inp["id"]))), expr)
...
def format_result(tool: Mapping[str, Any], total: float) -> str:
    decimals = int((tool["calculator"]["formula"] or {}).get("decimals") or 0)
    if decimals > 0:
